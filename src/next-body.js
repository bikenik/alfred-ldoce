		...
		for (let i = 0; i < $.run_ons.length; i++) {
			if ($.run_ons[i].examples) {
				items.push({
					...
					subtitle: $.run_ons[i].part_of_speech || $.run_ons[i].examples[0].text,
					arg: {
						definition: [
							`${$.run_ons[i].derived_form}<span class="neutral span"> [</span>${
			...
			if (sense.collocation_examples) {
				for (let y = 0; y < sense.collocation_examples.length; y++) {
					if (sense.collocation_examples[y].example.text !== undefined) {
						items.push({
							title: sense.collocation_examples[y].collocation || sense.definition[0],
							subtitle: sense.definition[0],
							arg: {
