...
for (let i = 0; i < mainData.length; i++) {
	...
	if (mainData[i].examples && mainData[i].examples[0].audio && mainData[i].headword === undefined) {
...
for (let i = 0; i < mainData.length; i++) {
	...
	if (data.examples && !data.examples[0].audio && data.headword === undefined) {
		// ${data.examples[x].audio[0].url}
		for (let x = 0; x < data.examples.length; x++) {
			let id = md5(data.examples[x].text)
			HTMLoutput += `<span class="EXAMPLE"><span class="speaker exafile fa fa-volume-up">[sound:${id}.mp3]</span>${data.examples[x].text}</span>`
